      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {services.map((service, idx) => (
            <motion.div
              key={service.id}
              className={`relative bg-gray-900 rounded-3xl p-6 flex flex-col justify-between shadow-2xl border-2 border-gray-800 hover:border-purple-500 transition-all duration-500 ${
            ...
            >
              {/* Popular Badge */}
              {service.popular && (
                <div className="absolute top-4 right-4 bg-gradient-to-r from-purple-500 to-pink-500 px-4 py-1 rounded-full text-sm font-bold text-black uppercase shadow-lg">
                  Most Popular
                </div>
              )}
